...
# runtime in apps/agent/iclone (that has its own venv + start.sh).
set -euo pipefail
cd "$(dirname "$0")/.."
...
NAME="$(node -p "require('./identity.json').marketplace_name" 2>/dev/null || echo 'iCLONE')"
say ""
